window.router.addRoute('home', async (container, params) => {
    ...
    container.innerHTML = `
        ...
        <div id="home-search-wrapper" style="display: flex; justify-content: center; background: white; padding: 0.4rem 1rem; border-bottom: 1px solid #eaeaea; position: relative; z-index: 20;">
            <div class="search-container" style="max-width: 350px; width: 100%; border: 1.5px solid #eaeaea; padding: 0.1rem 0.3rem; display: flex; align-items: center; border-radius: 99px; background: #fafafa; transition: all 0.3s ease;">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#999" stroke-width="2" style="margin-left:6px;"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
                <div style="flex:1;">
                    <input id="home-search-input" type="text" placeholder="Search hotels or cities..." style="border:none; padding:4px 8px; outline:none; background:transparent; width:100%; font-size:0.8rem;" value="${filterState.searchQuery}" onkeydown="if(event.key==='Enter') window.updateHomeSearch(this.value)">
                </div>
                <button class="btn-primary" style="border-radius:99px; padding:0.3rem 0.8rem; font-size:0.75rem;" onclick="window.updateHomeSearch(document.getElementById('home-search-input').value)">Search</button>
            </div>
        </div>
